...
def semantic_search(input:str)->str:
    ...
    prompt_template = ChatPromptTemplate.from_messages([
        ("system", """
    You are an expert in writing optimized Snowflake SQL queries."""
    ),
        ("user", """
    You are an expert Snowflake SQL generator. Given the following database semantic model yaml data, parse the data correctly to understand the database structure, and using that semantic model convert the user's question into an optimized Snowflake SQL query.

    Schema:
    {semantic_model}

    Carefully read the parsed yaml data, perform the below actions one after another:
    - Understand the intent and contex of the question asked by the user. the question asked will be in natural language.
    - try to understand which tables and columns are being referred to in the question. Use the ""description"" fields in the yaml data to understand the context of the columns.
    ...
    - No assumptions or speculations.
